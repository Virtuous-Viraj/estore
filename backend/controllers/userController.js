...
const updateUserProfile = async (req, res, next)=>{
    try {
        ...
        if (req.body.password !== user.password) {
            user.password = hashPasswords(req.body.password);
          }
          await user.save();
...
const writeReview = async(req, res, next) =>{
        try {
            ...
            // validate request :
            if(!(comment && rating)) return res.status(400).send("Review not proper all inputs required")
            // create review id manually coz it is also needed for saving in Product collection
...
const deleteUser = async(req, res, next) => {
        try {
            ...
            await User.deleteOne({_id : req.params.id}).orFail()
            // const result = User.deleteOne({id : req.params.id}).orFail()
            // console.log("hello")
            res.send("user removed")
                // res.status(500).send("Cannot perform the opearation")
