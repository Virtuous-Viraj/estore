...
const getProducts = async (req, res, next) => {
  try {
    ...
    if (categoryName) {
      ...
      // only those products for which category is like regular expression
...
const adminUpdateProduct = async (req, res, next) =>{
        try {
            ...
            await product.save()
            res.json({message : `Product with id : ${product._id} updated successfully`})
        } catch (error) {
            next(error)
        }
}
...
const adminDeleteProductImage = async(req, res, next) =>{
        try {
            ...
            // if u dont want to return any data
            const fs = require("fs")
            ...
            await Product.findOneAndUpdate({_id:req.params.productId}, {$pull : {images : {
                path : imagePath
            }}}).orFail()
            return res.end()
        } catch (error) {
